...
export class ProgramCounter {
  ...
  //   const pcCenterX =
  //     arrowTipX + offsetDistance * Math.cos((offsetAngle * Math.PI) / 180);
  //   const pcCenterY =
  //     arrowTipY + offsetDistance * Math.sin((offsetAngle * Math.PI) / 180);
  ...
  //       const angle =
  //         current.angle + (next.angle - current.angle) * localProgress;
  ...
  tryAdvance(): NodeSingular | null {
    if (this.currentEdge != null) {
      ...
      if (getTerminalProgramCounters(dest).size === 0) {
        ...
        getTerminalProgramCounters(this.currentLocation).delete(this.id);
