...
function MakeFileSelectedHandler(levelContext: LevelContext) : (event : React.ChangeEvent<HTMLInputElement>) => Promise<void>{
...
export function SaveLoadControls({context}: {context: LevelContext}) {
    ...
    <h3>Graph</h3>
        <div className="controls">
            <button id="saveBtn" className="control-btn" onClick={() => downloadGraph(context)}>💾 Save</button>
            <button id="loadBtn" className="control-btn" onClick={() => fileInputRef?.current?.click()}>📂 Load</button>
            <input ref={fileInputRef} type="file" onChange={MakeFileSelectedHandler(context)} id="fileInput" accept=".json" style={{display: "none"}}/>
        </div>
        </> );
}