...
function createStartNode(x: number, y: number): void {
...
function createStopNode(x: number, y: number): void {
    const nodeId = `node-${nodeIdCounter++}`;
...
function createPlusNode(x: number, y: number): void {
    const nodeId = `node-${nodeIdCounter++}`;
...
function createCombineNode(x: number, y: number): void {
    const nodeId = `node-${nodeIdCounter++}`;
    const input1Id = `${nodeId}-in1`;
    const input2Id = `${nodeId}-in2`;
    const outputId = `${nodeId}-out`;
...
function createSplitNode(x: number, y: number): void {
    const nodeId = `node-${nodeIdCounter++}`;
    const inputId = `${nodeId}-in`;
...
function createNopNode(x: number, y: number): void {
    const nodeId = `node-${nodeIdCounter++}`;
    const inputId = `${nodeId}-in`;
    const outputId = `${nodeId}-out`;
...
function setupNodeDeletion(): void {
...
function initializePreviews(): void {
